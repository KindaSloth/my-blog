...
console.log(
  printer.printNode(ts.EmitHint.Unspecified, typeStringAlias, undefined)
);
```
...
console.log(
  printer.printNode(ts.EmitHint.Unspecified, unionTypeAlias, undefined)
);
```
...
console.log(
  printer.printNode(ts.EmitHint.Unspecified, interfaceDeclaration, undefined)
);
```
...
console.log(printer.printNode(ts.EmitHint.Unspecified, obj, undefined));
```
...
const parse = <T extends Record<string, T>>(
  ...
  if (Array.isArray(json)) {
    ...
    if (typeof json[0] === "object") {
      ...
      fs.appendFile(
        "./output.ts",
        printer.printNode(ts.EmitHint.Unspecified, typeDeclaration, undefined),
        function (err) {
          if (err) console.log("error", err);
        }
      );
